...
3. (Opcional, se tem acesso a Bash) Salve a peça final em arquivo temporário e rode `python3 scripts/quality_gate.py {arquivo} --type {post|artigo|email|landing-page|anuncio}` para lint operacional (acentos, hook strength, CTA, formato)
...
Todas as capacidades acima são sumários. **Para profundidade**, SEMPRE consulte `subagents/copy-agent.md`: 3510 linhas cobrindo:
