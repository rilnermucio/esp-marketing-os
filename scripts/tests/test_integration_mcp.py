...
class TestPipelineMOSExecucao:
    ...
    def test_execucao_seo_analyze_chama_subprocess(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            with patch(
                "sys.argv", ["mos.py", "seo", "analyze", "artigo.md", "keyword"]
            ):
                with pytest.raises(SystemExit) as exc:
                    main()
    ...
    def test_execucao_hooks_generate_chama_subprocess(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            with patch(
                "sys.argv",
                ["mos.py", "hooks", "generate", "produtividade", "reels", "10"],
            ):
                with pytest.raises(SystemExit) as exc:
                    main()
            assert exc.value.code == 0
    ...
    def test_execucao_quality_check_chama_subprocess(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0)
            with patch(
                "sys.argv", ["mos.py", "quality", "check", "post.md", "--type", "post"]
            ):
                with pytest.raises(SystemExit) as exc:
                    main()
            assert exc.value.code == 0
...
class TestCoberturaDeSscripts:
    ...
    def test_nao_ha_scripts_fantasma_no_command_map(self):
        ...
        assert (
            not fantasmas
        ), f"COMMAND_MAP referencia scripts inexistentes: {fantasmas}"
...
class TestIntegracoesDiAPI:
    ...
    def test_gsc_analyzer_importavel(self):
        ...
        assert hasattr(gsc_analyzer, "GSCError") or hasattr(
            gsc_analyzer, "GSCAnalyticsError"
        )
    ...
    def test_instagram_api_tem_constantes_de_env(self):
        ...
        assert hasattr(instagram_api, "ENV_ACCESS_TOKEN") or any(
            "TOKEN" in k for k in vars(instagram_api) if isinstance(k, str)
        )
    ...
    def test_meta_ads_api_tem_versao(self):
        ...
        tem_versao = any(
            "VERSION" in k
            or "version" in k.lower()
            or (
                isinstance(v, str) and ("v1" in v or "v2" in v) and "graph" in v.lower()
            )
...
class TestIntegridadeScripts:
    ...
    def test_scripts_tem_shebang_python(self):
        ...
        for script in SCRIPTS_PATH.glob("*.py"):
            ...
            if not (
                primeira_linha.startswith("#!")
                or script.name in {"validators.py", "output_formatter.py"}
            ):
        ...
        for script in SCRIPTS_PATH.glob("*.py"):
            ...
            if primeira_linha.startswith("#!"):
                assert (
                    "python" in primeira_linha.lower()
    ...
    def test_todos_scripts_tem_docstring_de_modulo(self):
        ...
        assert not sem_docstring, f"Scripts sem docstring de módulo:\n" + "\n".join(
            sem_docstring
        )
    ...
    def test_pytest_ini_ou_setup_cfg_existe(self):
        ...
        assert any(
            f.exists() for f in [pytest_ini, setup_cfg, pyproject]
