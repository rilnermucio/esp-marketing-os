...
def analyze_content(content: str, keyword: Optional[str] = None) -> Dict:
    ...
    if keyword:
        ...
        in_h1 = bool(
            re.search(
                rf"^#\s.*{re.escape(keyword_lower)}", content.lower(), re.MULTILINE
            )
        )
        in_h2 = bool(
            re.search(
    ...
    if word_count < 300:
        recommendations.append(
            "⚠️ Conteúdo curto (<300 palavras). Para SEO, considere expandir para 1000+ palavras."
        )
    elif word_count < 1000:
        recommendations.append(
            "📝 Conteúdo médio. Para competir em SEO, considere expandir para 1500+ palavras."
        )
    else:
    ...
    if headers["h1"] == 0:
        ...
        recommendations.append(
            "⚠️ Apenas um H1 por página. Encontrados: " + str(headers["h1"])
        )

    if headers["h2"] < 2:
        recommendations.append(
            "📝 Adicione mais H2s para estruturar o conteúdo (recomendado: 3-5)."
        )

    if avg_sentence_length > 25:
        recommendations.append(
            "⚠️ Sentenças muito longas (média: {:.1f} palavras). Ideal: <20 palavras.".format(
                avg_sentence_length
            )
        )
    ...
    if external_links == 0:
        recommendations.append(
            "📝 Adicione links externos para fontes confiáveis (E-E-A-T)."
        )

    if keyword_analysis:
        if keyword_analysis["density"] < 1:
            recommendations.append(
                f"⚠️ Densidade de keyword baixa ({keyword_analysis['density']}%). Aumente uso de '{keyword}'."
            )
        elif keyword_analysis["density"] > 2.5:
            recommendations.append(
                f"⚠️ Possível keyword stuffing ({keyword_analysis['density']}%). Reduza uso de '{keyword}'."
            )
    ...
    return {
        ...
        "seo_score": calculate_seo_score(
            word_count, headers, keyword_analysis, external_links
        ),
    }
...
def main() -> None:
    ...
    try:
        filepath = validar_arquivo(
            sys.argv[1], extensoes=[".md", ".txt"], campo="arquivo"
        )
        keyword = None
        if len(sys.argv) > 2:
    ...
    print("\n🏗️ ESTRUTURA:")
    print(
        f"   • Headers: H1={result['structure']['headers']['h1']}, H2={result['structure']['headers']['h2']}, H3={result['structure']['headers']['h3']}"
    )
    ...
    if result["keyword_analysis"]:
        ...
        print(
            f"   • Nas primeiras 100 palavras: {'✅' if result['keyword_analysis']['in_first_100_words'] else '❌'}"
        )
