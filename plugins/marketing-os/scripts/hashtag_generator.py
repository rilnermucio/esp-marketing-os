...
PLATFORM_LIMITS = {
    "instagram": {
        "max": 30,
        "recommended": 10,
        "note": "Use 5-15 hashtags misturando popularidade",
    },
    "linkedin": {
        "max": 5,
        "recommended": 3,
        "note": "Apenas hashtags profissionais e específicas",
    },
    "twitter": {"max": 2, "recommended": 2, "note": "Apenas 1-2 hashtags relevantes"},
    "tiktok": {
        "max": 5,
        "recommended": 4,
        "note": "Hashtags trending + nicho específico",
    },
    "facebook": {
        "max": 3,
        "recommended": 2,
        "note": "Hashtags opcionais, foco no conteúdo",
    },
}


def get_hashtags(
    nicho: str, platform: str = "instagram", custom_keywords: Optional[List[str]] = None
) -> Dict:
    ...
    platform_config = PLATFORM_LIMITS.get(
        platform.lower(), PLATFORM_LIMITS["instagram"]
    )
...
def main() -> None:
    ...
    try:
        ...
        custom_keywords = (
            [validar_texto(k, campo="keyword", max_len=50) for k in sys.argv[3:]]
            if len(sys.argv) > 3
            else None
        )
